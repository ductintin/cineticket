...
export default function Seat() {
  ...
  const [time, setTime] = useState(searchParam.get("date") || "");
  const [scheduleId, setScheduleID] = useState(
    searchParam.get("scheduleId") || ""
  );
  const [seatArr, setSeatArr] = useState<any[]>([]);
  const price = [50000, 100000];
 

  const theatreName = searchParam.get("theatreName") || "";
  ...
  useEffect(() => {
    ...
    const timeParam = searchParams.get('date');
  ...
  const fetchScreen = async () => {
    ...
    const data = await screenAPI.getScreenByScheduleIdAndTime(
      user?.token,
      scheduleId,
      time
    );
  ...
  const setBookedSeatss = async function (id: string, data: any) {
    setBookedSeats([]);
    const res = await screenAPI.setBookedSeat(
      screenInfo._id,
      data,
      user?.token
    );
    ...
    if (arr[0] !== seatArr.length - 1) {
      setSelectedSeats((prevSelectedSeats) => {
        if (prevSelectedSeats.some((ar) => ar.toString() === arr.toString())) {
          ...
          setTotalPrice(
            (selectedSeats.length - 1) * price[0] +
              selectedDoubleSeats.length * price[1]
          );

          return prevSelectedSeats.filter(
            (id) => id.toString() !== arr.toString()
          );
        } else {
  ...
  const handleBookingBtn = async () => {
    ...
    const res1 = screenAPI.createReservation(
      userId,
      newReservation,
      user?.token
    );
    setTimeout(async () => {
      const res2 = screenAPI.bookReservation(
        (await res1).data._id,
        user?.token
      );
        ...
        <div className={styles.body}>
          <div className={styles.movie_container}>
            <div className={styles.container}>
                    ...
                    {row.map((seat: any, index: number) => (
                      <div
                        ...
                        className={`${
                          ...
                          selectedSeats.some(
                            (arr) =>
                              arr.toString() === [indexrow, index].toString()
                          ) ||
                          selectedDoubleSeats.some(
                            (arr) =>
                              arr.toString() === [indexrow, index].toString()
                          )
                            ? styles.selected
              ...
              <p className={styles.text}>
                Số lượng ghế đôi là{" "}
                <span id="countdouble">{selectedDoubleSeats.length}</span>
              </p>
              <p className={styles.text}>
                Vị trí ghế là{" "}
                <span id="address">
                  {selectedSeats
                    .map((seatId) => {
                      const seat = seatArr.find(
                        (seat) => seat[0] === seatId[0] && seat[1] === seatId[1]
                      );

                      return `${convert2Alphabet(seatId)[0]}${
          ...
          <div className={styles.lay1}>
            ...
            <div className={styles.mvDetails1}>
              <div className={styles.input}>
                ...
                />
                {vietnamBankAccountPattern.test(bankAccountNumber) ? (
                  <p>Số tài khoản hợp lệ!</p>
                ) : (
                  <h6>Số tài khoản không hơp lệ!</h6>
                )}
              </div>

              <button className={styles.ripple} onClick={() => handleBackBtn()}>
                QUAY LẠI
              </button>
